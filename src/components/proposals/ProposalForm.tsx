...
/**
 * Shared create/edit form for proposals. Both /dashboard/proposals/new and
 * /dashboard/proposals/[id]/edit render this so the two screens can never drift
 * apart (the bug that left "edit" linking to a page that never existed). The
 * page owns persistence (POST vs PATCH + redirect); this component owns the
 * fields, line-item math, and validation.
 */
...
export function emptyProposalForm(): ProposalFormData {
  return {
    client_id: '',
    title: '',
    description: '',
    line_items: [{ description: '', quantity: 1, rate_cents: 0, amount_cents: 0 }],
    valid_until: '',
    project_overview: '',
    project_start_date: '',
...
interface ProposalFormProps {
  ...
  /** Persist the proposal. Throw an Error to surface a message to the user. */
...
export default function ProposalForm({
  ...
  const [formData, setFormData] = useState<ProposalFormData>(initialData ?? emptyProposalForm());

  // Seed the per-row dollar inputs from any existing line items (cents -> dollars).
  const [rateInputs, setRateInputs] = useState<Record<number, string>>(() => {
    const seed: Record<number, string> = {};
    formData.line_items.forEach((item, i) => {
      seed[i] = item.rate_cents ? (item.rate_cents / 100).toString() : '';
    });
    return seed;
  });

  // "Draft with AI": turn a short brief into a filled-in proposal.
  const [aiBrief, setAiBrief] = useState('');
  const [aiLoading, setAiLoading] = useState(false);
  const [aiError, setAiError] = useState<string | null>(null);

  const seedRateInputs = (items: ProposalLineItem[]) => {
    const seed: Record<number, string> = {};
    items.forEach((it, i) => {
      seed[i] = it.rate_cents ? (it.rate_cents / 100).toString() : '';
    });
    setRateInputs(seed);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/proposals/generate', {
        ...
        body: JSON.stringify({ brief: aiBrief, client_name: clientName }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to draft proposal');
      ...
      setAiError(err instanceof Error ? err.message : 'Failed to draft proposal');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const updateLineItem = (index: number, field: keyof ProposalLineItem, value: string | number) => {
  ...
  return (
    <div className="px-3 py-4 sm:p-6 max-w-4xl mx-auto">
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6">
        <label className="block text-sm font-semibold text-emerald-900 mb-1">Draft with AI</label>
        <p className="text-xs text-emerald-700 mb-2">
          Describe the project in a sentence or two and AI fills in the title, overview, line items, and
          terms. Review and edit everything before sending.
        </p>
        <textarea
          className="w-full rounded-lg border px-3 py-2 text-sm"
          rows={3}
          placeholder="e.g. Catalog website for a wine importer, around $3k, three phases (design, build, launch)..."
          value={aiBrief}
        ...
        <button
          ...
          disabled={aiLoading}
          className="mt-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 text-sm font-medium"
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          <Link href="/dashboard/proposals" className="px-4 py-2 text-gray-600 hover:text-gray-800">
            Cancel
          </Link>
          <button
            type="submit"
